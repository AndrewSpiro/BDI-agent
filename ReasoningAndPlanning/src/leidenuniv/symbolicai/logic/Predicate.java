...
public class Predicate {
    ...
    public void parse(String sfol) {
        ...
        // No need to go through this unless you want to understand how parsing goes, or
        // want to extend your predicate
        try {
            sfol = sfol.trim();

            Pattern p = Pattern.compile("\\(\\w*(,\\w*)*\\)");// this is the regular expression that looks for
                                                              // (term,term,...)
    ...
    private void parsePred(String pred) {
        ...
        // It figures out if the predicate is a reserved predicate; either a comparison
        // (!= or =) or an operator (an addition or deletion to the BDI databases), and
        // then parses the name
        if (pred.equals("!=")) {
            not = true;
            this.name = "";
        } else if (pred.equals("=")) {
    ...
    public boolean not() {
        // Check if this is a special predicate ! and check for X!=Y, we use !(X,Y)
        // prefix notation
    ...
    public boolean eql() {
        // Check if this is a special predicate =, to check for X=Y, we use =(X,Y)
        // prefix notation
        // Returns true if term1==term2
    ...
    public boolean bound() {
        // Checks if this predicate is bound, i.e., a predicate that is fully
        // instantiated contains no variables.
        if (hasTerms()) {
            ...
            return true;
        } else // no terms means this predicate is bound, because it does not have any
               // variables
            return true;
    }
    ...
    public boolean isAction() {
        ...
        // The intended semantic is that such operators add or remove/facts to/from the
        // intention, desire, or belief databases of the agent.
    ...
    public String toString() {
        ...
        // This is an important function as it is not only used for printing but also
        // for generating copies of predicates
        // and to compare predicates with each other for unification (lexographically
        // identical)
        // Behavior: new
        // Predicate(pred.toString()).toString().equals(pred.toString())==true
        // It creates a copy of pred and string compares that copy with the original
        // always resulting in true.
        String result = (neg ? "!"
                : adopt ? "*" : drop ? "~" : act ? "_" : not ? "!=" : eql ? "=" : add ? "+" : del ? "-" : "") + name;
        if (terms.size() > 0) {
            result += "(";
            for (Term t : terms) {
                result += t.toString() + ",";
